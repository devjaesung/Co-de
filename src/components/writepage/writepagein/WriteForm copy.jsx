...
const WriteForm = () => {
  ...
  const addInput = (e) => {
    ...
    newSpan.innerHTML = "<label><input type='checkbox' name='color' value='blue'> Blue</label>";
    addinput.appendChild(newSpan);
  }
      ...
      <Box className="d-basicbox">
        ...
        <FormGroup row sx={{p:"10px 30px"}}>
        {
          ...
          <Input type="text" placeholder="태그를 추가해보세요" id="addInput"
            sx={{fontSize:"14px", width:"150px", padding:"4px 10px"}}/>
          <button className="addbtn">추가하기</button>
          {/* <button className="addbtn" onClick={addInput}>
            <Add sx={{verticalAlign:"-6px", color:"#555"}} />태그를 추가해보세요
          </button> */}
        </FormGroup>
      </Box>
      <Box className="d-basicbox">
